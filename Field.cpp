...
void Field::checkOverlap(T &group1, U &group2, int size1, int size2, int &done)
{
    for (int i = 0; i < size1; ++i) // check collision between two objects saved in an array
    {
        for (int j = 0; j < size2; ++j)
        {
            if (i != j || typeid(T) != typeid(U))
            {
                if (group1[i].sprite.getGlobalBounds().intersects(group2[j].sprite.getGlobalBounds()))
                {
                    setNewPosition(group1[i].sprite); // move object to new position if collision was detected
                    done++;
                }
            }
        }
...
template <typename U>
void Field::checkOverlapP(Player &player, U &group1, int size1, int &done) // overload for player and object
{
    for (int i = 0; i < size1; ++i)
    {
        if (group1[i].sprite.getGlobalBounds().intersects(player.sprite.getGlobalBounds()))
        {
            setNewPosition(group1[i].sprite);
            done++;
        }
    }
}
...
template <typename T>
void Field::checkOverlapPC(PokeCenter &pokeCenter, T &group1, int size1, int &done)
{
    for (int i = 0; i < size1; ++i)
    {
        if (group1[i].sprite.getGlobalBounds().intersects(pokeCenter.sprite.getGlobalBounds()))
        {
            setNewPosition(group1[i].sprite);
            done++;
        }
...
void Field::setNewPosition(sf::Sprite &entity)
{
    ...
    entity.setPosition(newPosX, newPosY);            // set new position
    window_field->draw(entity);
}
