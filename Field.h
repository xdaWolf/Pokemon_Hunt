...
private:
    ...
    void checkOverlap(T &pGroup1, U &pGroup2, int pSize1, int pSize2, int &pDone);
    template <typename U>
    void checkOverlapP(Player &pPlayer, U &pGroup1, int pSize1, int &pDone);
    template <typename T>
    void checkOverlapPC(PokeCenter &pPokeCenter, T &pGroup1, int pSize1, int &pDone);
    void setNewPosition(sf::Sprite &pEntity);
